...
type GuestProfileForm = z.infer<typeof guestProfileSchema>;
...
export default function GuestPage() {
  ...
  const onSubmit = async (data: GuestProfileForm) => {
    setIsLoading(true);
    setError('');
    ...
    try {
      ...
      localStorage.setItem('token', guestResponse.data.access_token);

      // Create guest profile
      await profilesAPI.createGuest({
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 placeholder-black bg-white opacity-100"
                style={{ color: '#000' }}
                placeholder="Describe your skills, technologies, and experience (e.g., Python, React, AWS, 3 years of experience in web development)"
              />
              {errors.skills && (
            ...
            <div className="flex items-center justify-between pt-6">
              ...
              >
                {isLoading ? 'Creating Profile...' : 'Create Profile & Start Interview'}
              </button>
            </div>
          </form>
        </div>
